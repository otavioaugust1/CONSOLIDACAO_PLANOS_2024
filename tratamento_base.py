...
# Download do arquivo SAIPS
url = 'https://www.gov.br/saude/pt-br/composicao/saes/saips/manuais/manuais-drac/plano-de-atendimento-per_cir-eletiva-versao-8.xlsm/@@download/file'
filename = pasta_entrada + f'BASE_DE_DADOS_SAIPS_V8.xlsm'
with urllib.request.urlopen(url) as response, open(filename, 'wb') as out_file:
    data = response.read()  # Lê os dados do arquivo
    out_file.write(data)  # Escreve os dados em um arquivo local
print(f"[OK] Download do arquivo BASE_DE_DADOS_SAIPS.xlsm ===========================>: {time.strftime('%H:%M:%S')}")
...
dado_sigtap = glob('BASE/BASE_DE_DADOS_SIGTAP_202*.zip')[0] 
...
## SIGTAP - PLANILHA
df_planilha_proc = glob('BASE\BASE_DE_DADOS_SAIPS*.xlsm')[0] # Listando os arquivos do diretório
...
df_sigtap_f = pd.merge(df_planilha_proc, df_sigtap_modalidade, left_on='CO_PROCEDIMENTO', right_on='CO_PROCEDIMENTO', how='left') # Juntando os arquivos 3
...
df_sigtap_f['EXIGE_HABILITACAO'] = np.where(df_sigtap_f['CO_HABILITACAO'].notnull(),'SIM','-') # Exige Habilitação (sim ou não)
df_sigtap_f['EXIGE_SERVIÇO'] = np.where(df_sigtap_f['CO_SERVICO'].notnull(),'SIM', '-') # Exige Serviço (sim ou não)
